...
export async function upsertPswapLineage(dbId, orderId, originalPswap, currentTipNoteId, currentDepth, remainingOffered, remainingRequested, state, createdAtBlock, updatedAtBlock, creatorAccountId, assetPairTag, subscriptionAnchorNoteId) {
    try {
...
export async function getPswapLineage(dbId, orderId) {
    try {
        const db = getDatabase(dbId);
...
export async function listPswapLineages(dbId, filter) {
    try {
        const db = getDatabase(dbId);
        if (filter === "Active") {
 ...
 */
export async function applyPswapRound(dbId, payload) {
    const db = getDatabase(dbId);
    return await db.dexie.transaction("rw", db.pswapLineages, db.inputNotes, db.notesScripts, db.tags, async (tx) => {
        const row = await tx.pswapLineages.get(payload.orderId);
        if (!row) {
            throw new Error(`apply_pswap_round: unknown order_id ${payload.orderId}`);
        }
        ...
        if (payload.roundDepth !== row.currentDepth + 1) {
            throw new Error(`apply_pswap_round: round_depth ${payload.roundDepth} does not advance current_depth ${row.currentDepth} by 1`);
        }
        ...
        for (const note of payload.inputNotes) {
            ...
            if (!existing) {
                await upsertInputNote(dbId, note.detailsCommitment, note.noteId, note.noteAssets, note.attachments, note.serialNumber, note.inputs, note.noteScriptRoot, note.noteScript, note.nullifier, note.createdAt, note.stateDiscriminant, note.state, note.consumedBlockHeight, note.consumedTxOrder, note.consumerAccountId, tx);
            }
        }
        if (payload.state === STATE_FULLY_FILLED ||
