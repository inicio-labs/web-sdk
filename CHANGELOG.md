...
* [FEATURE][web,react] Added PSWAP order tracking. The client now persists a *lineage* per swap order — the chain of remainder notes a partially-fillable swap leaves behind as it is filled round-by-round — keyed by a stable `orderId`. A new `pswap` resource on `MidenClient` exposes the reads `pswap.lineages()`, `pswap.lineagesFor(creator)`, and `pswap.lineage(orderId)`, returning `PswapLineageRecord`s (offered/requested assets, remaining amounts, current depth + tip note id, and `PswapLineageState` of `Active` / `FullyFilled` / `Reclaimed`), plus `pswap.cancelByOrder(orderId)`, which builds and submits a cancel against the order's current tip. Four React hooks expose this: `usePswapLineages`, `usePswapLineagesFor`, `usePswapLineage`, and `usePswapCancelByOrder` (cancel by stable `orderId`, resolving the creator + current tip from the tracked lineage). ([#PR-B](https://github.com/0xMiden/web-sdk/pull/PR-B), companion: [0xMiden/miden-client#2231](https://github.com/0xMiden/miden-client/pull/2231)).
